...
//#endif
	...
	Object obj;
	obj.VAO = loadSimpleOBJ(R"(C:\Users\Luanm\Documents\demo\CG2024-2\4.5-Hello3D-OBJ\Suzanne.obj)",obj.nVertices);

	if (obj.VAO == 0) {
		cout << "Erro ao carregar o OBJ!" << endl;
	} else {
		cout << "OBJ carregado com sucesso! Numero de vertices: " << obj.nVertices << endl;
	}
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glDrawElements(GL_TRIANGLES, obj.nVertices, GL_UNSIGNED_INT, nullptr);
		glBindVertexArray(0);
...
int loadSimpleOBJ(string filePath, int &nVertices) {
    ...
    glm::vec3 color = glm::vec3(1.0, 0.0, 0.0);  // Define cor padrão
    ...
    while (getline(arqEntrada, line)) {
        ...
        if (word == "v") {
            ...
            while (ssline >> word) {
                ...
                // Adiciona os vértices ao buffer
    ...
    // Configuração do atributo posição (x, y, z)
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(GLfloat), (GLvoid*)0);
    glEnableVertexAttribArray(0);

    // Configuração do atributo cor (r, g, b)
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(GLfloat), (GLvoid*)(3 * sizeof(GLfloat)));
    glEnableVertexAttribArray(1);

    // Desvincula o buffer de vértice e o VAO
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    glBindVertexArray(0);

    nVertices = vBuffer.size() / 6;  // Cada vértice tem 3 posições + 3 cores
    return VAO;
}
